                ...
                columns: [
                    {
                        ...
                        render: (data, type, row) => {
                            ...
                            <button type="button" class="btn btn-danger" onclick="cancelReservation('${row.r_id}','${row.room_id}','${row.room_number}','${row.start_date}','${row.end_date}','${row.start_time}','${row.end_time}','${row.c_name}','${row.c_email}','${row.phone_num}','${row.rp_price}','${row.rp_rate_description}','${row.transaction_id}')"><i class="fa fa-trash" aria-hidden="true"></i></button>
                            `
                        }
                    },
                    { data: 'transaction_id' },
...
<form id="customForm" method="POST">
  ...
  function() {
    ...
    alertify.success('Customer reservation successfully  logged.');
...
function cancelReservation(id, room_id, room, start_date, end_date, start_time, end_time, c_name, email, number, price, desc,transaction_id) {

alertify.confirm(
  'Cancel Log Reservation',
  `
<form id="cancelcustomForm" method="POST">
    <div>
      <label for="input1">Reservation ID:</label>
      <input type="text" name="input1" id="input1" class="form-control" value="${transaction_id}" readonly/>
    </div>
    ...
    <div>
    <label for="input3">Reason:</label>
    <select name="cancellationReason" class="form-select" id="cancellationReason">
    <option value="Date not valid">Date is not valid</option>
    <option value="Date in past">Date is in the past</option>
    </select>
    </div>
    <input type="text" name="r_id" id="r_id" class="form-control" value="${id}" hidden readonly/>
</form>
  ...
  function() {
    ...
    // Serialize the form data
    var formData = $('#cancelcustomForm').serialize();
